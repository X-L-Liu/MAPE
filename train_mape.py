...
def create_train_loader():
    train_AE_path = [os.path.join(data_path, classifier_name[random.randint(0, len(classifier_name) - 1)]
                                  + '-' + att + '-AE-' + str(random.randint(1, config.data_amount)) + '.pkl')
...
def train_model(SAU_Net_, Optimizer):
    ...
    with tqdm(total=len(train_loader), desc='Train-Progress') as pbar:
        for k, (image, label) in enumerate(train_loader):
            ...
            logit = classifiers[random.randint(0, len(classifier_name) - 1)](image_ - image_pert)
    ...
    with torch.no_grad():
        ...
        with tqdm(total=len(test_loader), desc='Test--Progress') as pbar:
            for k, (image, label) in enumerate(test_loader):
                ...
                logit = classifiers[0](image_ - image_pert)
                top1.update((logit.max(1)[1] == label).sum().item(), len(label))
                pbar.update(1)
    return top1.acc_rate
...
def main(SAU_Net_, OptimizerAdam, OptimizerSgd, Reload):
    ...
    for epoch in range(config.epochs):
        ...
        if test_acc > config.best_acc:
            torch.save(SAU_Net_.state_dict(), os.path.join(net_save_path, f'SAU_Net_MAPE_{test_acc}.pt'))
            sau_net_load_path = os.path.join(net_save_path, f'SAU_Net_MAPE_{test_acc}.pt')
            if os.path.exists(os.path.join(net_save_path, f'SAU_Net_MAPE_{config.best_acc}.pt')):
                os.remove(os.path.join(net_save_path, f'SAU_Net_MAPE_{config.best_acc}.pt'))
...
if __name__ == '__main__':
    ...
    classifier_name = []
    for cla in classifier_load_path.keys():
        if classifier_load_path[cla] is not None:
            classifier_name.append(cla)

    test_AE_path = [os.path.join(data_path, classifier_name[0] + '-' + att + '-AE-1.pkl') for att in
    ...
    for cla_name in classifier_name:
        ...
        cla.load_state_dict(torch.load(classifier_load_path[cla_name], map_location=config.device))
        cla.to(device)
        cla.eval()
