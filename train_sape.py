...
def train_model():
    ...
    with tqdm(total=len(train_loader), desc='Train-Progress') as pbar:
        for k, (image, label) in enumerate(train_loader):
            ...
            for n in range(3):
                image[n::4] = attacks[n](image[n::4], label[n::4])
            logit = classifier(image - net(image))
...
def main(Reload):
    ...
    for epoch in range(config.epochs):
        ...
        if test_acc > config.best_acc:
            net_load_path = os.path.join(net_save_path, f'SAPE_CAU_Net_{test_acc*100:.2f}.pt')
...
if __name__ == '__main__':
    ...
    classifier.eval()
    attacks = [torchattacks.FGSM(classifier), torchattacks.PGD(classifier), torchattacks.DIFGSM(classifier)]
